...
class RasterioFileTileSource(FileTileSource, metaclass=LruCacheMetaclass):
    ...
    def __init__(self, path, projection=None, unitsPerPixel=None, **kwargs):
        ...
        :param unitsPerPixel: The size of a pixel at the 0 tile size.  Ignored
            ...
            converted to the projection divided by the tile size.  crs
        ...
        computed_level = math.ceil(max(logX, logY) / math.log(2))
        self.sourceLevels = self.levels = int(max(0, computed_level) + 1)
    ...
    def _getLargeImagePath(self):
        ...
        ('http', 'https', 'ftp', 's3') for use with fiona
        ...
        supported_scheme = {'http', 'https', 'ftp', 's3'}
        if urlparse(path).scheme in supported_scheme:
            if path.startswith('s3://'):
                s3_path = path.replace('s3://', '')
                path = f'/vsis3/{s3_path}'
            else:
                rasterio_options = {
                    'url': path,
                    'use_head': 'no',
    ...
    def _scanForMinMax(self, dtype, frame=0, analysisSize=1024, onlyMinMax=True):
        ...
        has_min = all(b.get("min") is not None for b in bandInfo.values())
        has_max = all(b.get("max") is not None for b in bandInfo.values())
        if not frame and onlyMinMax and has_max and has_min:
            with self._getDatasetLock:
        ...
        range_max = np.iinfo(dtype).max if isinstance(dtype, np.integer) else 1
        self._bandRanges[frame]['max'] = np.append(self._bandRanges[frame]['max'], range_max)
    ...
    def getLRUHash(*args, **kwargs):
        ...
        return super(RasterioFileTileSource, RasterioFileTileSource).getLRUHash(*args, **kwargs) + info
    ...
    def getBounds(self, crs=None):
        ...
        dst_crs = pyproj.crs.CRS(crs) if crs is not None else crs
        
        # exit if it's already set 
        if dst_crs.srs in self._bounds:
        ...
        self._bounds[dst_crs.srs] = bounds
    ...
    def getMetadata(self):
        ...
        with self._getDatasetLock:
            ...
            # rasterio is only designed to deal with geospatial images. If nothing can be found it will raise
            # an error anyway (TODO: check this assertion)
    ...
    def _bandNumber(self, band, exc=True):
        ...
        is_band_number = band != -1 and band not in bands
        band = band if is_band_number else None
    ...
    def getTile(self, x, y, z, pilImageAllowed=False, numpyAllowed=False, **kwargs):
        ...
        if not self.projection:
            ...
            if xmin >= bounds['xmax'] or xmax <= bounds['xmin'] or ymin >= bounds['ymax'] or ymax <= bounds['ymin']:
        ...
        return self._outputTile(tile, TILE_FORMAT_NUMPY, x, y, z, pilImageAllowed, numpyAllowed, **kwargs)

    def _convertProjectionUnits(self, left, top, right, bottom, width, height, units, **kwargs):
        """
    ...
    def _getRegionBounds(self, metadata, left=None, top=None, right=None,
        ...
        projection.  If units is a proj string, use that projection.  Otherwise, just use the super
        function.
        ...
        """
        is_units = units is not None
        units = TileInputUnits.get(units.lower() if is_units else None, units)
        ...
        if units == 'projection' and self.projection:
            ...
            if top is None:
                top = bounds['ymax'] if bottom is None or height is None else bottom - width
            if bottom is None:
    ...
    def getThumbnail(self, width=None, height=None, **kwargs):
        ...
        if self.projection:
            ...
            no_width = width is not None and width < 2
            no_height = height is not None and height < 2
            if no_width or no_height:
    ...
    def toNativePixelCoordinates(self, x, y, crs=None, roundResults=True):
        """
        Convert a coordinate in the native projection to
        pixel coordinates.
        ...
        src_crs = self.projection if crs is None else pyproj.crs.CRS(crs)
            ...
            if 0 <= int(x) < self.sizeX and 0 <= int(y) < self.sizeY:
                with self._getDatasetLock:
                    for i in range(self.dataset.count):
                        ...
                        try:
                            value = self.dataset.read(i+1, window=window).astype(np.single)
    ...
    def _encodeTiledImageFromVips(self, vimg, iterInfo, image, **kwargs):
        ...
        convertParams = large_image.tilesource.base._vipsParameters(defaultCompression='lzw', **kwargs)
        convertParams.pop('pyramid', None)
        
        is_compressed = convertParams['compression'] in {'webp', 'jpeg'}
        vimg = large_image.tilesource.base._vipsCast(vimg, is_compressed)
        ...
        with tempfile.NamedTemporaryFile(suffix=".tiff", prefix="tiledRegion_") as f:
            ...
            with rio.open(f.name) as src, tempfile.NamedTemporaryFile(suffix=".tiff", prefix="tiledGeoRegion_", delete=False) as output:
    ...
    def getRegion(self, format_=(TILE_FORMAT_IMAGE, ), **kwargs):
        ...
        if not (iterInfo and not self._jsonstyle and TILE_FORMAT_IMAGE in format_ and kwargs.get('encoding') == 'TILED'):
        ...
        with self._getDatasetLock, tempfile.NamedTemporaryFile(suffix=".tiff", prefix="tiledGeoRegion_", delete=False) as output: 
    ...
    def validateCOG(self, strict=True, warn=True):
        ...
        is_valid, errors, warnings = cog_validate(self._largeImagePath, strict=strict)
