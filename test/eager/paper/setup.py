...
def print_directory_slide_dimensions(file_dir: str, file_extensions: list[str] = [
                                     '.tif', '.svs', '.mrxs', '.ndpi'], scale: Optional[dict] = None, tile_size: Optional[dict] = None):
        ...
        for file in files:
            if file.endswith(tuple(file_extensions)):
                ...
                slide_dimensions = calculate_slide_dimensions(
                    source, scale=scale, tile_size=tile_size)
                print(
                    f'{file_path}: \n {json.dumps(metadata, indent=4)} \n {json.dumps(slide_dimensions, indent=4)}')


def copy_random_files_from_file_directory(file_dir: str, n_files: int = 10, file_extensions: list[str] = [
                                          '.tif', '.svs', '.mrxs', '.ndpi'], destination_dir: str = './test_files', large_image_meta_match: Optional[dict] = None):
