...
def _assertImageMatches(image, testRootName, saveTestImageFailurePath="/tmp"):
    ...
    testImagePaths = glob.glob(
        os.path.join(testDir, "test_files", testRootName + "*.png")
    )
    testImages = [
        PIL.Image.open(testImagePath).convert("RGBA")
...
def testTileFromGeotiffs():
    ...
    assert tileMetadata["bounds"]["srs"] == "EPSG:32611"
...
def testTileStyleBadInput():
    ...
    _assertStyleResponse(
        imagePath,
        {"band": 1, "palette": "nonexistent.palette"},
        "cannot be used as a color palette",
    )

    _assertStyleResponse(imagePath, ["style"], "Style is not a valid json object.")
...
def testPixel():
    ...
    pixel = source.getPixel(
        region={"left": -13132910, "top": 4010586, "units": "projection"}
    )
    assert pixel == {
        "r": 94,
        "g": 98,
    ...
    # Test with styles
    style = {"band": 1, "min": 0, "max": 100, "palette": "matplotlib.Plasma_6"}
    source = large_image_source_rasterio.open(
        imagePath, projection="EPSG:3857", style=style
    )
    pixel = source.getPixel(
    ...
    source = large_image_source_rasterio.open(imagePath, projection="EPSG:3857")
    pixel = source.getPixel(
        region={"left": -13132910, "top": 4010586, "units": "EPSG:3857"}
    )
    assert pixel == {
        "r": 94,
        "g": 98,
...
def testStereographicProjection():
    ...
    with pytest.raises(TileSourceError, match="unitsPerPixel must be specified"):
        large_image_source_rasterio.open(imagePath, "EPSG:3411")
    # But will pass if unitsPerPixel is specified
    large_image_source_rasterio.open(imagePath, "EPSG:3411", unitsPerPixel=150000)
...
def testConvertProjectionUnits():
    ...
    tsProj = large_image_source_rasterio.open(imagePath, projection="EPSG:3857")
...
def testGuardAgainstBadLatLong():
    ...
    source = large_image_source_rasterio.open(imagePath)
    bounds = source.getBounds(crs="EPSG:4326")
...
def testGetRegionWithProjection():
    ...
    region, _ = ts.getRegion(
        output=dict(maxWidth=1024, maxHeight=1024), format=constants.TILE_FORMAT_NUMPY
    )
...
def testGetTiledRegion():
    ...
    region, _ = ts.getRegion(
        output=dict(maxWidth=1024, maxHeight=1024), encoding="TILED"
    )
    result = large_image_source_rasterio.open(str(region))
...
def testGetTiledRegionWithProjectionAndStyle():
    ...
    imagePath = datastore.fetch("landcover_sample_1000.tif")
    ts = large_image_source_rasterio.open(
        imagePath, projection="EPSG:3857", style='{"bands":[]}'
    )
    # This gets the whole world
    region, _ = ts.getRegion(
        output=dict(maxWidth=1024, maxHeight=1024), encoding="TILED"
