...
def get_input_params_from_user(algorithm, sig):
    ...
    print(
        "Hit enter to accept default for every run, provide a single value to replace the default for every run."
    )
    print(
        "Provide two values separated by a comma to specify a range to cover across all runs."
    )
    input_params = {}
